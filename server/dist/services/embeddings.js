...
function wordPieceTokenize(text, maxLen = 128) {
...
export async function downloadEmbeddingModel() {
    ...
    for (const { url, dest } of files) {
        ...
        if (!resp.ok)
            throw new Error(`Failed to download ${name}: ${resp.statusText}`);
...
function meanPooling(lastHiddenState, attentionMask, seqLen, hiddenSize) {
...
export async function embed(text) {
    ...
    const lastHiddenState = results[outputName].data;
